...
function savePlanning(){
  ...
  st.plan=next;st.need=n('plan-need');persist();return true;
}
...
renderChart=function(){
  ...
  const model=RetirementCalculator.fromState(st),profile=RetirementCalculator.getRiskProfile(model),profileComparisons=RetirementCalculator.compareProfiles(model).map(p=>({...p,base:RetirementCalculator.simulateCapitalDevelopment(RetirementCalculator.withReturnProfile(model,p.profile.key))}));
  document.querySelector('.chart-context').textContent=`Profil: ${profile.label}${profile.custom?' · individuell angepasst':''} · reale Zielrendite ${(profile.expectedRealReturn*100).toLocaleString('de-DE',{minimumFractionDigits:1,maximumFractionDigits:2})} % · ${TaxModel.canton(st.canton)?'nach Entnahmen und geschätzten Steuern':'nach Entnahmen, Steuern noch nicht berücksichtigt'} · in heutiger Kaufkraft`;
  ...
  const comparisonValues=profileComparisons.flatMap(p=>[...p.unfavourable,...p.favourable,...p.base].filter(row=>row.age<=comparisonEnd).map(available));
  const rawMax=Math.max(1,...comparisonValues,...pessimistic.map(available),...optimistic.map(available),...base.map(available)),tickUnit=rawMax>900000?100000:50000,max=Math.ceil(rawMax/(tickUnit*3))*tickUnit*3;
  ...
  document.getElementById('devPotSummary').innerHTML=[
    ['bond','Topf 2','Anleihen',optimistic[0].buckets[1]],['cash','Topf 1','Geldmarkt',optimistic[0].buckets[0]],['stock','Topf 3','Wachstum',optimistic[0].buckets[2]]
  ...
  let comparison=document.getElementById('profileComparison');
  if(!comparison){comparison=document.createElement('section');comparison.id='profileComparison';document.getElementById('gapPoint').after(comparison);}
  comparison.innerHTML=`<h3>Verfügbares Restkapital mit Alter ${input.end}</h3><p>Profilvergleich zum selben Zielalter wie die Grafik · alle drei Töpfe, ohne Immobilien. ${profile.custom?'Hier gelten die Standardprofile; deine individuellen Annahmen siehst du in der Grafik. ':''}Die Basis zeigt konstante Renditen; die Varianten eine günstige oder ungünstige Reihenfolge. Höheres Risiko garantiert kein besseres Ergebnis.</p><div class="profile-comparison">${profileComparisons.map(p=>`<article data-comparison="${p.profile.key}"><strong>${p.profile.label}${!profile.custom&&p.profile.key===profile.key?' · gewählt':''}</strong><span>Basisrechnung<b data-baseline>${CHF(p.base.at(-1).free)}</b></span><span>Ungünstige Reihenfolge<b data-unfavourable>${CHF(p.unfavourable.at(-1).free)}</b></span><span>Günstige Reihenfolge<b data-favourable>${CHF(p.favourable.at(-1).free)}</b></span></article>`).join('')}</div><small>Separat in Immobilien gebunden: ${CHF(input.bound)}. Marktwert minus Hypotheken; in den dargestellten Kapitalverläufen nicht enthalten.</small>`;
};
