...
function renderPlanningForm(){
  ...
  target.innerHTML=`<fieldset ${planningLocked?'disabled':''}><div class="kicker">Lebensplanung & Entnahme</div><h2>Dein Bedarf in drei Phasen</h2><p class="note">Beträge in heutiger Kaufkraft, inklusive Wohnkosten, ohne Einkommenssteuern. Diese werden separat vom Einkommen abgezogen.</p>
...
function renderPlanOverview(refreshFinancing=true){
  ...
  renderTaxResults(first);rIncomeTotal.textContent=CHFJ(first.rent);rRent.textContent=CHFJ(first.rent);rYield.closest('.row').classList.add('hidden');rOther.closest('.row').classList.add('hidden');
  ...
  if(incomeRows[1])incomeRows[1].querySelector('span').textContent='Netto verfügbare Einnahmen';
...
renderChart=function(){
  ...
  gapPoint.innerHTML='<span aria-hidden="true">↕</span><div>Die Unterschiede entstehen durch die Reihenfolge derselben Renditen. Gezeigt ist nur der Aktientopf nach geplanten Entnahmen und geschätzten Steuern.</div>';
