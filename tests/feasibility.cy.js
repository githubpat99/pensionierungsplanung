describe('Feasibility before PK choice',()=>{
  ...
  it('separates overall feasibility from the current split and keeps the selected state',()=>{
    ...
    cy.get('#planFeasibility').should('have.class','shortfall').and('contain','Finanzierungslücke');
    cy.window().then(w=>{
    ...
    cy.get('#planFeasibility').should('contain','Unter den gewählten Annahmen');
    cy.get('#planFeasibility').should('have.class','feasible').and('contain','bis Alter 95 finanzierbar');
    ...
    cy.get('#planFeasibility').should('have.class','shortfall').and('contain','voraussichtlich bis Alter');
    cy.viewport(390,844);
