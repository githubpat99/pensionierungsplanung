describe('Tax planning integration',()=>{
  it('persists canton, updates PK split, uses net capital, and supports retired plans',()=>{
    cy.visit('/');
    cy.window().then(w=>{
      w.eval("st=structuredClone(D);st.mode='pre';st.currentAge=65;st.retirementAge=65;st.assets.pk=1000000;st.pkShare=100;ui();jump('ageNow');");
    });
    cy.get('#groupEditor [name=canton]').should('have.value','').find('option').should('have.length',27);
    ...
    cy.window().then(w=>{
      ...
      w.eval("st.mode='post';st.post.free=650000;ui();CheckUI.go('income');");
    ...
    cy.viewport(390,844);
    cy.window().then(w=>w.eval("CheckUI.go('capital')"));
