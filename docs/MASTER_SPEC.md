...
Säule 3a ist im Startkapital vollständig enthalten. Eine gesonderte 3a-Bezugssteuer wird derzeit nicht berechnet; diese Modellgrenze muss offengelegt werden. Post werden keine künftigen PK-/3a-Beiträge oder Bezugsentscheide abgefragt. In V2 wird die tatsächlich laufende PK-Rente ausschliesslich unter Vorsorge → Pensionskasse erfasst; im Einkommensbereich nur angezeigt.
...
**IMPLEMENTIERT – Menü und PK-Bearbeitung:** Das Hamburger-Menü ist in allen V3-Ansichten erreichbar und enthält «Persönliche Angaben → AHV-Renten → Pensionskasse → Weitere Einnahmen → Bedarf → Vermögen → Annahmen». Persönliche Angaben bearbeiten Zeitpunkt, Vor-/Nach-Pensionierung und Wohnkanton; AHV und weitere Einnahmen bearbeiten getrennte Ausschnitte derselben Einkommensgruppe. Alle Gruppeneditoren kehren nach Übernahme, Abbruch oder «← Mein Plan» zum Plan zurück. Vor vollständigem Einstieg führt der Rückweg zuerst zu fehlenden Zeitpunkt-/Bedarfsangaben. Menüereignisse werden einmal gebunden; Escape schliesst das Menü.

«Pensionskasse (PK)» ist die einzige V3-Bearbeitungsseite für PK-Grunddaten. Vor Pensionierung verwendet sie die vorhandenen Felder für Guthaben und gemeinsame Sparbeiträge sowie Verzinsung und Umwandlungssatz. Datenbasis bleibt `CheckV2State`: Bestände/Beiträge unter `details.pension`, PK-Raten unter `details.assumptions`. Diese Raten werden unter «Annahmen» nicht ein zweites Mal editiert. Der Kapitalanteil ist auf der PK-Seite nur Kontext; die Entscheidung und die Auswahl der festen Vergleichspunkte bleiben unter «Mein Plan». Die Seite ist keine reine Kapitalansicht: Sie zeigt unter den Eingaben die gesamte PK-Situation der aktuellen Variante als berechnete, nicht editierbare Werte – die aktuell gewählte Aufteilung («xx % Rente / yy % Kapital»), die laufende PK-Rente pro Monat und pro Jahr sowie das PK-Kapital mit Brutto, geschätzter Bezugssteuer und Netto. Rente und Kapital stammen aus demselben `calculatePension`-Aufruf wie «Mein Plan» und können deshalb nicht voneinander abweichen; es gibt keinen zweiten PK-Auswahlzustand im Detail.

Die beiden PK-Kennzahlen auf dem Plan sind Buttons. Der Renten-Button öffnet dieselbe PK-Seite und setzt Fokus und Scrollposition auf den PK-Überblicksabschnitt mit der hervorgehobenen PK-Rente; der Kapital-Button öffnet denselben Abschnitt mit hervorgehobenem PK-Kapital. Die Aufschlüsselung stammt aus `calculatePension`; keine editierbaren Brutto-/Steuer-/Nettoergebnisse. Ohne Kanton zeigt der Plan ausdrücklich «PK-Kapital brutto» mit «Steuern offen»; im Detail sind Steuer und Netto offen. Der Wohnkanton wird unter «Persönliche Angaben» ergänzt. Mit Kanton stimmen Betrag, Satz und ausgeschriebener Kanton mit dem gemeinsamen Steuerrechner überein.

**IMPLEMENTIERT – V3-Navigation «Mein Plan» als Hub:** Jeder direkt aus «Mein Plan» erreichbare Wert öffnet seinen fachlich passenden Abschnitt und scrollt ihn in den sichtbaren Bereich: AHV, weitere Renten und weitere Einnahmen die jeweiligen Eingabefelder, Bedarf das Bedarfsfeld, «Verfügbares Vermögen» die Vermögenstotal-Zeile, der Plan-Kopf «Persönliche Angaben» und Pensionskasse die beiden PK-Kennzahlen (siehe oben). Der Sprung erfolgt synchron nach dem Rendern über `scrollIntoView` mit `scroll-margin-top` in Höhe der tatsächlich gerenderten Sticky-Header-Höhe; auf einen Timer wird verzichtet. Der Zielabschnitt erhält kurz einen dezenten, per `animationend` wieder entfernten Fokusrahmen und wird programmatisch fokussiert (`tabindex="-1"`, `prefers-reduced-motion` respektiert). Bestehende Screens werden wiederverwendet; es entsteht kein zusätzlicher Screen pro Wert.

Alle V3-Detail-Screens verwenden dasselbe Kopfmuster: eine kompakte, beim Scrollen oben klebende Leiste mit «← Mein Plan» links und dem Detailtitel daneben (Desktop und Mobile identisch). Dies ist die einzige primäre Rücknavigation eines Detail-Screens; ein zusätzlicher «Mein Plan»-Button am Seitenende entfällt (entfernt aus «Varianten vergleichen»). Die Formularaktionen «Abbrechen» und «Übernehmen» bleiben davon getrennt am Formularende. «← Mein Plan» und «Abbrechen» verwerfen unbestätigte Eingaben, «Übernehmen» schreibt sie und kehrt mit aktuellen Werten zum Plan zurück. Die Scrollposition von «Mein Plan» wird beim Verlassen gemerkt und bei der Rückkehr wiederhergestellt.

PK-Eingaben liefern im Detail eine ungespeicherte Vorschau aus demselben Rechner; erst «Übernehmen» schreibt die PK-Grunddaten. Danach werden Plan und Vergleichspunkte mit den gemeinsamen geänderten PK-Daten neu berechnet. Gewählter Kapitalanteil, Horizont, übrige Einkünfte und Anlageannahmen bleiben erhalten. PK-Änderungen bestätigen nicht automatisch alle Annahmen. Abbruch und Rücklink verwerfen die unbestätigte Eingabe. Speichern/Laden erhält die bestätigten Zustände; ein geladener vollständiger Plan öffnet direkt «Mein Plan».
