...
**IMPLEMENTIERT – V3-Variantenvergleich (verdichtet):** Der Vergleich beginnt mit dem Seitentitel und einer kurzen Subline («So entwickelt sich dein Kapital bis Alter …», Zielalter dynamisch). Die frühere separate Zeile «Deine Wahl: x % PK-Kapital» entfällt; die Angabe steht kompakt als «x % PK-Kapital · Deine Wahl» direkt unter dem Kartentitel «Kapitalentwicklung ⓘ». Der Umschalter «Alle Varianten» sitzt im Kopf derselben Karte (Schalterdarstellung, `role="switch"`, id `compareLines`) und ersetzt die frühere eigene Checkbox-Zeile. Die Grafik ist deutlich flacher (mobil rund 200 px, auf Desktop grosszügiger) und passt sich der Containerbreite an; die Y-Achse ist kompakt beschriftet («1.6 Mio.», «500 k»), die X-Achse zeigt weiterhin echte Alterswerte mit derselben Tick-Ausdünnung wie bisher (der Zielalter-Tick hat Vorrang vor einem regulären Fünfjahresschritt). Statt einer zweiten Wertzeile unter dem Diagramm trägt jeder Verlauf seinen Endwert direkt an der Linie; die interaktiven Punktziele und die Live-Wertzeile entfallen ersatzlos, die Grafik behält ihren `aria-label`. Direkt unter der Grafik steht die kompakte Legende der dargestellten Verläufe (gewählte Variante zuerst, gleiche Farben wie die Linien). Die PK-Aufteilung folgt unmittelbar darunter mit dem Titel «PK-Aufteilung im Vergleich ⓘ» und der kurzen Subline «Monatliche Rente und Kapital zu Beginn (Alter …).»; die frühere mehrzeilige Erklärung sowie der Hinweis zu einer frei gewählten Quote liegen hinter dem ⓘ. Die drei Karten 0/50/100 % bleiben erhalten und sind kompakt: Titel und «✓ Deine Wahl» teilen sich eine Zeile, darunter stehen PK-Rente pro Monat, verfügbares Startkapital und Kapital am Planungshorizont in drei Spalten (mobil innerhalb der Karte, Desktop nebeneinander). Der Rückweg «← Mein Plan» sitzt ohne grosse Leerfläche direkt am Inhaltsende. Am Desktop darf der Vergleich mehr Breite nutzen; die fachliche Logik ist für Mobile und Desktop identisch.

**IMPLEMENTIERT – V3-Screen «Jahr für Jahr»:** Ein eigener Screen erklärt die bestehende Jahressimulation an den echten Planungsdaten; er ist aus dem Variantenvergleich (und aus den Steuerdetails auf «Mein Plan») über «So funktioniert deine Planung Jahr für Jahr →» erreichbar. Aufbau: Titel, Subline «So entwickelt sich dein Geld über die Jahre.», kompakte PK-Variantenauswahl als kurzer Dropdown (0 % · Rente / 50 % · Rente + Kapital / 100 % · Kapital; die gewählte Quote als Badge «✓ Deine Wahl» neben dem Feld), Jahresnavigation über Slider plus `‹ Vorjahr` / `Nächstes Jahr ›`, dann das gewählte Jahr als kompakte Schrittliste mit **genau einer Zahl pro Schritt**: Einnahmen (vor Steuern), Steuern (mit Abzug), Einnahmen netto, Bedarf, Offen (aus Vermögen, zusätzlich pro Monat), im Bezugsjahr Kapitalbezug netto, die drei Töpfe zu Jahresbeginn, Rendite, Auffüllen auf die Zielstruktur und Kapital Ende Jahr; dazu die aufklappbare Detailberechnung («Berechnung dieses Jahres ansehen») und «Weiter zu Alter X →». Alle Herleitungen (Einnahmen je Quelle, steuerbares Einkommen und Satz, brutto/Steuer/netto der Bezüge, Zielwerte der Töpfe, Rendite je Topf mit Satz, Umbuchungen, Töpfe am Jahresende) liegen hinter dem ⓘ des jeweiligen Schritts – eine zweite Jahreszusammenfassung im Seitenfluss gibt es nicht. «Jahr abspielen» zeigt die Schritte nacheinander (respektiert `prefers-reduced-motion`), «Gesamten Verlauf» läuft durch die Jahre. Alle Beträge kommen aus `evaluatePlan().yearlyProjection` und `capitalWithdrawalEvents()` derselben Planung – es gibt keine zweite Rechnung im UI. Damit die Jahresansicht die Einnahmen je Quelle und die Rendite je Topf zeigen kann, liefert die Engine zusätzlich je Jahr `sources` (Quelle, brutto, steuerbar) und `gains` (Ertrag je Topf); beide Werte entstehen in `retirement-engine.js` aus denselben Grössen wie die übrige Rechnung.

**AUDITIERT – Abweichungen der Engine von der «Jahr für Jahr»-Spezifikation (bewusst, nicht stillschweigend geändert):** (1) **Obligationen-Ziel:** Die Spezifikation nennt «2 × jährlicher Kapitalbedarf». Die Engine füllt den Geldmarkttopf exakt mit dem Kapitalbedarf des laufenden Jahres und den Obligationentopf mit dem **Kapitalbedarf der beiden Folgejahre** (`reserve`); bei gleichbleibendem Bedarf ist das identisch, bei steigendem Bedarf etwas höher (= die Zielstruktur «1 Jahr / 2 Jahre» wird effektiv abgedeckt). Die UI nennt deshalb die Untertitel «1 Jahr», «2 Jahre» und «Rest» und legt im ⓘ beide Zielwerte offen. Eine Umstellung auf «2 × laufender Bedarf» wäre eine Änderung der Berechnung und braucht einen eigenen Auftrag. (2) **Rebalancing:** Es gibt keine explizite Sequenz «Obligationen → Geldmarkt, dann Wertschöpfung → Obligationen»; die Töpfe werden zu Beginn jedes Jahres nach derselben Priorität neu aus dem Gesamtkapital gebildet (Geldmarkt zuerst, dann Obligationen, Rest Wertschöpfung). Das Ergebnis entspricht der Auffüllreihenfolge; die tatsächlichen Bewegungen zeigt die UI über `transfers`. (3) **Mischjahr:** Ein Pensionierungsjahr mit Erwerbs- und Renteneinkommen ist nicht modelliert; das erste Jahr ist ein volles Rentenjahr. Die UI kennzeichnet es als «Steuern im ersten Jahr bereits berücksichtigt ✓» und nennt im ⓘ beide Punkte (Spezifikationstext und die Modellvereinfachung). (4) **3a-Zuordnung:** Der Standard der Spezifikation (Sicherheitstöpfe zuerst füllen, Rest in die Wertschöpfung) ist exakt die Prioritätsregel der Engine; eine wählbare Alternativverteilung ist nicht umgesetzt. (5) **Bezugssteuern:** Sie sind kein Cashflow im Jahreslauf, sondern werden im Bezugsjahr beim Nettozufluss abgezogen (PK beim Startkapital, 3a beim Jahreszufluss); die UI weist sie im Bezugsjahr als brutto − Steuer = netto aus. (6) Alle Jahreswerte sind in heutiger Kaufkraft, die Steuerbemessung rechnet nominal.

**IMPLEMENTIERT – V3-Renditen und Mechanik offengelegt:** Die Annahmen-Seite weist keine separat einstellbare Wertschriftenrendite mehr aus (kein Feld `secReturn`, keine Nennung als Nutzerannahme); das Feld bleibt aus Kompatibilität im State und wird für den Aufbau vor der Pensionierung intern verwendet. An seiner Stelle steht das ⓘ «So rechnen wir mit Renditen» und erklärt: bis zur Pensionierung wachsen PK-Guthaben mit der PK-Verzinsung, Säule 3a mit der 3a-Rendite und Wertschriften mit dem hinterlegten internen Satz (Standard 4,5 %, dynamisch aus dem State gelesen); im Ruhestand liegt das Kapital in drei Töpfen – Cash 0 %, Anleihen 1 %, Wertschöpfung mit der realen Rendite des Risikoprofils (Standard 6 %). Im Cash-Topf liegt die Entnahme des laufenden Jahres, in den Anleihen die Entnahmen der nächsten zwei Jahre, der Rest in der Wertschöpfung; die Aufteilung wird jedes Jahr aus dem verbleibenden Kapital neu gebildet. Die Entnahme eines Jahres ist der Bedarf abzüglich Renten nach geschätzter Einkommenssteuer; die Kapitalbezugssteuer wird einmalig beim Bezug abgezogen und danach nicht erneut belastet. Alle Beträge sind in heutiger Kaufkraft gerechnet, die Renten sind feste Nominalbeträge ohne Teuerungsausgleich.

**AUDITIERT – bewusst akzeptierte Modellgrenzen (Stand V3):** (1) Es gibt kein explizites Rebalancing und keine Wiederauffüllungsregel; die jährliche Neubildung der Töpfe aus dem Gesamtkapital wirkt als implizites Rebalancing ohne Transaktionskosten. (2) Die code-seitige Entnahmereihenfolge Cash → Anleihen → Wertschöpfung ist buchhalterisch wirkungslos, weil der Cash-Topf immer genau auf die Jahresentnahme gesetzt wird; die Entnahme wird damit faktisch vollständig aus dem Cash-Topf bedient. (3) Die Ansparphase rechnet mit nominalen Sätzen (PK-Verzinsung, 3a-Rendite, interner Wertschriftensatz), die Ruhestandsphase mit realen Portfolio-Sätzen; die Mischung bleibt bewusst bestehen, weil der Plan nominale Renten und heutige Kaufkraft nebeneinander als Standortbestimmung zeigt. (4) Die laufende Einkommenssteuer wird je Variante und Jahr neu aus dem variantenspezifischen steuerbaren Einkommen gerechnet; in den Testdaten bleibt der Satz für 0/50/100 % bei 10,5 % (alle unter CHF 80'000), die Progression greift erst bei hohen PK-Guthaben. (5) Risiko und Stressszenarien sind nicht Teil des Variantenvergleichs; die Karten zeigen die Basisrechnung. Diese Punkte sind dokumentiert, aber nicht umgesetzt; Änderungen daran brauchen einen eigenen fachlichen Auftrag.
