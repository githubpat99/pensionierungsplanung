...
Der V3-Hauptfluss lautet «Meine Renten → Mein Bedarf → Mein Plan». Der Plan enthält die PK-Entscheidung als Kapitalanteil von 0 bis 100 Prozent mit Regler. Eine Vorschau verändert den aktuellen Plan erst nach «Variante übernehmen». Bis zu drei Anteile können gespeichert und mit derselben Zeit- und Wertskala verglichen werden. Der aktive Plananteil wird separat von den drei gespeicherten Varianten und der ausgewählten Vorschau erhalten; er ist nicht zwingend die erste Variante.
...
«Pensionskasse» ist die einzige V3-Bearbeitungsseite für PK-Grunddaten. Vor Pensionierung verwendet sie die vorhandenen Felder für Guthaben und gemeinsame Sparbeiträge sowie Verzinsung und Umwandlungssatz. Datenbasis bleibt `CheckV2State`: Bestände/Beiträge unter `details.pension`, PK-Raten unter `details.assumptions`. Diese Raten werden unter «Annahmen» nicht ein zweites Mal editiert. Der Kapitalanteil ist auf der PK-Seite nur Kontext; die Entscheidung und drei Variantenspeicher bleiben unter «Mein Plan».

Die PK-Kapitalkennzahl auf dem Plan ist ein Button. Er öffnet dieselbe PK-Seite wie das Menü und setzt Fokus und Scrollposition auf «PK-Kapital brutto → geschätzte Kapitalbezugssteuer → PK-Kapital netto» für den zuvor angezeigten Anteil. Die Aufschlüsselung stammt aus `calculatePension`; keine editierbaren Brutto-/Steuer-/Nettoergebnisse. Ohne Kanton zeigt der Plan ausdrücklich «PK-Kapital brutto» mit «Steuern offen»; im Detail sind Steuer und Netto offen. Der Wohnkanton wird unter «Persönliche Angaben» ergänzt. Mit Kanton stimmen Betrag, Satz und ausgeschriebener Kanton mit dem gemeinsamen Steuerrechner überein.

PK-Eingaben liefern im Detail eine ungespeicherte Vorschau aus demselben Rechner; erst «Übernehmen» schreibt die bestehende Datengruppe. Danach werden Plan und alle Varianten mit den gemeinsamen geänderten PK-Daten neu berechnet. Gespeicherte Anteile, aktiver Plan, ausgewählte Vorschau, Horizont, übrige Einkünfte und Anlageannahmen bleiben erhalten. PK-Änderungen bestätigen nicht automatisch alle Annahmen. Abbruch und Rücklink verwerfen die unbestätigte Eingabe. Speichern/Laden erhält diese Zustände; ein geladener vollständiger Plan öffnet direkt «Mein Plan».
...
**IMPLEMENTIERT – Diagrammlesbarkeit:** Die Diagrammgeometrie passt sich der tatsächlichen Containerbreite an; Achsenbeschriftungen bleiben auch auf Mobile 14 px gross und kontrastreich. Ein eigener linker Rand trennt vollständige CHF-Beträge von Kurven und Markern. Das quadratische Variantensymbol in Plankarten und Vergleichslegende ist einheitlich 7 × 7 px gross.
