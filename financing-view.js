  ...
  document.getElementById('planOverview').innerHTML = `
    ...
    <div class="finance-layout">
      ...
      <button class="primary finance-mobile finance-open" onclick="document.getElementById('financeDialog').showModal()">Simulation anpassen</button><dialog id="financeDialog" class="finance-dialog"><button class="secondary finance-mobile finance-close" onclick="document.getElementById('financeDialog').close()">Fertig · Zur Finanzierung</button><section class="finance-panel finance-controls"><div class="finance-sim-title"><h3>Simulation</h3><details class="finance-info"><summary aria-label="Information zur Simulation">ⓘ</summary><p>Beträge in heutiger Kaufkraft. Der Jahresbedarf wird je Lebensphase gespeichert. Markt, Dauer und Startkapital sind Vorschauwerte. Die Topfbeträge zeigen den Jahresanfang der gewählten Phase, der Endwert das verbleibende Kapital nach allen geplanten Entnahmen.</p></details></div><div class="finance-sim-fields">
        ...
        <div class="finance-sim-field"><label for="financeCapital">Startkapital <output id="financeCapitalOutput"></output></label><input id="financeCapital" type="range" min="0" max="${Math.max(5000000,p.capital)}" step="10000" value="${p.capital}" oninput="changeFinancing('capital',this.value)"></div>
...
function updateFinancingValues() {
  ...
  document.getElementById('financeProjection').innerHTML=`<strong>Endkapital mit ${last.age}: ${CHF(last.free)}</strong><span>${gap?`Erste ungedeckte Lücke: Alter ${gap.age}–${gap.age+1}.`:'Bis zum Zielalter gedeckt.'}</span>${phaseUnavailable?'<span>Die gewählte Phase liegt ausserhalb des Zeithorizonts. Darstellung: Planungsstart.</span>':''}`;
  const income=p.sources.map(s=>({name:s.name,value:RetirementEngine.simulate({...p,sources:[s]},f.scenario).find(x=>x.age===age).rent,icon:s.id==='ahv'?'▥':s.id==='pk'?'▣':s.rental?'⌂':'•••'})).filter(s=>s.value!==0);
  document.getElementById('financeIncome').innerHTML=`<h3>Renten & weitere Einnahmen</h3><p>Diese Einnahmen decken einen Teil deines Jahresbedarfs ab Alter ${age}.</p><div class="finance-source-list">${income.map(s=>`<div class="finance-source"><span class="finance-source-icon" aria-hidden="true">${s.icon}</span><strong>${planningEscape(s.name)}</strong><div><b>${CHF(s.value)}</b><small>pro Jahr</small></div></div>`).join('')||'<p>Keine laufenden Einnahmen erfasst.</p>'}</div><div class="finance-total"><strong>Total Einnahmen</strong><div><b>${CHF(shown.rent)}</b><small>pro Jahr</small></div></div><div class="finance-income-arrow" aria-hidden="true">Deckt einen Teil des Bedarfs →</div>`;
  ...
  document.getElementById('financeIncome').insertAdjacentHTML('afterbegin',`<div class="finance-mobile finance-compact-title"><strong>Einnahmen · ${CHF(shown.rent)}/J.</strong>${info('Information zu Einnahmen','Laufende Einnahmen ab Alter '+age+', in heutiger Kaufkraft. Die Laufzeiten und Inflationsannahmen stehen unter Annahmen.')}</div>`);
