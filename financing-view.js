...
function updateFinancingValues() {
  ...
  document.getElementById('financeModel').innerHTML=`<h3>So fliesst dein Geld durch die drei Töpfe</h3>${p.capital===p.capitalBreakdown.totalInvestableCapital?investableCapitalSummary(p):`<p>Gesamtes Anlagekapital · Vorschau: <strong>${CHF(p.capital)}</strong></p>`}<p>Aufteilung zu Jahresbeginn mit ${age}: <strong>${CHF(shown.free)}</strong></p><div class="finance-refill"><span>Jährlich prüfen und die Töpfe bedarfsgerecht ausgleichen</span></div><div class="finance-pots"><svg class="finance-flow-arrows" viewBox="0 0 600 90" preserveAspectRatio="none" aria-hidden="true"><defs><marker id="bondArrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10Z" fill="#ed8c91"/></marker><marker id="stockArrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10Z" fill="#71afe7"/></marker></defs><path d="M140 64 Q195 7 250 50" fill="none" stroke="#ed8c91" stroke-width="6" marker-end="url(#bondArrow)"/><path d="M460 64 Q300 -25 140 50" fill="none" stroke="#71afe7" stroke-width="6" marker-end="url(#stockArrow)"/></svg>${pot(1,'bond-pot','Anleihen','Reserve für die folgenden zwei Entnahmejahre. Wird aus Topf 3 aufgefüllt.')}${pot(0,'cash-pot','Geldmarkt','Laufende Entnahmen eines Jahres. Wird aus Topf 2 aufgefüllt.')}${pot(2,'stock-pot','Aktien','Verbleibendes Kapital für spätere Jahre.')}</div><div class="finance-payout">↓<br><strong>Auszahlungen für deinen Lebensunterhalt</strong></div>${shown.gap>0?`<p class="note gold">Das Kapital reicht in diesem Jahr nicht aus. Ungedeckt: ${CHF(shown.gap)}.</p>`:''}<p class="finance-model-note">Aktienquote: ${shown.free?(shown.buckets[2]/shown.free*100).toFixed(1):'0'} %. Die Aktienquote ergibt sich aus dem verbleibenden Anlagekapital, nachdem die geplanten Entnahmen in Topf 1 und Topf 2 reserviert wurden. Modellrechnung; jährliche Auffüllung kann auch nach Kursverlusten Aktienverkäufe erfordern.</p>`;
