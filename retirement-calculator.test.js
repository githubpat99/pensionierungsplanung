...
const planning=fs.readFileSync('retirement-planning.js','utf8');
...
for(const mode of ['pre','post'])for(const share of [0,50,100]){
 ...
 assert.deepEqual(JSON.parse(JSON.stringify(input)),old,`${mode}/${share}: same capital, income, tax inputs and periods`);
