...
function renderFinancingView(reset = true) {
  ...
  document.getElementById('planOverview').innerHTML = `
    ...
    <div class="finance-layout">
      <section class="finance-panel finance-funding" id="financeFunding" aria-label="Finanzierungsübersicht"></section>
...
function updateFinancingValues() {
  ...
  const fundingRow=(label,value,id,extra='')=>`<div class="finance-funding-row" id="${id}"><div class="finance-funding-label">${label}${extra}</div><div class="finance-funding-values"><strong>${CHF(value)} <small>/ Jahr</small></strong><span>${CHF(value/12)} <small>/ Monat</small></span></div></div>`;
  document.getElementById('financeFunding').innerHTML=`<small class="finance-funding-age">Ab Alter ${age}</small>${fundingRow('Bedarf',shown.need,'financeNeed')}${fundingRow(shown.estimatedIncomeTax===null?'Einkommen · vorläufig ohne Steuerabzug':'Einkommen netto',shown.rent,'financeIncome','<button type="button" class="finance-help-button" data-income-details aria-label="Einkommen und Steuern im Detail ansehen">ⓘ</button>')}${fundingRow('Aus Vermögen',shown.withdrawal,'financeWithdrawal')}`;
