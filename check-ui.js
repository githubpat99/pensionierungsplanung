...
window.CheckUI=(()=>{
 ...
 const routes={
  ...
  income:{title:'Einkommen & Steuern',parent:'capital',view:'overview'},
 ...
 const editorRoutes={ageNow:'edit_personal',horizon:'assumptions',income:'edit_income',postIncome:'edit_income',need:'edit_income',assets:'edit_assets',postAssets:'edit_assets',build:'edit_pension',pk:'pension'};
 ...
 function editOverview(){
  ...
  const rows=[
   ['edit_personal',[['Situation',post?'Bereits pensioniert':'Vor Pensionierung'],['Alter heute',`${st.currentAge} Jahre`],...(!post?[['Pensionierung',`Mit ${st.retirementAge}`]]:[]),['Wohnsitzkanton',TaxModel.canton(st.canton)?.name||'Noch nicht gewählt']]],
 ...
 function editor(){
  ...
  let fields='';
  if(state==='edit_personal')fields=input('currentAge','Alter heute',st.currentAge,'Jahre','min="18" max="100" step="1"')+(!post?input('retirementAge','Pensionierungsalter',st.retirementAge,'Jahre','min="50" max="100" step="1"'):'')+cantonField();
 ...
 function submitGroup(form){
  ...
  if(state==='edit_personal'){
   next.currentAge=n('currentAge');next.retirementAge=post?Math.min(next.retirementAge,next.currentAge):n('retirementAge');next.canton=data.get('canton');
   if(!post&&next.retirementAge<next.currentAge){shell.querySelector('#checkFormError').textContent='Das Pensionierungsalter darf nicht vor deinem heutigen Alter liegen.';return;}
   LifeExpectancy.updateAutomatic(next);
   if(next.planningAge<=(post?next.currentAge:next.retirementAge)){shell.querySelector('#checkFormError').textContent='Dein gespeicherter Horizont liegt vor dem Planungsstart. Bitte zuerst unter «Lebensphasen & Annahmen» verlängern.';return;}
  }
 ...
 function submit(form){
  ...
  if(!post&&next.retirementAge<next.currentAge){document.getElementById('checkFormError').textContent='Das Pensionierungsalter darf nicht vor deinem heutigen Alter liegen.';return;}
   LifeExpectancy.updateAutomatic(next);
   if(next.planningAge<=(post?next.currentAge:next.retirementAge)){shell.querySelector('#checkFormError').textContent='Dein gespeicherter Horizont liegt vor dem Planungsstart. Bitte zuerst unter «Lebensphasen & Annahmen» verlängern.';return;}
 ...
 shell.addEventListener('click',event=>{
  ...
  if(button.dataset.mode){st.mode=button.dataset.mode;if(st.mode==='post'){st.currentAge=Math.max(st.currentAge,st.retirementAge);}if(st.mode==='post'||st.retirementAge>=st.currentAge)LifeExpectancy.updateAutomatic(st);ensurePlan();persist();go('basic');}
 ...
 function closeFinanceHelp(){
  const panel=document.getElementById('financeHelp');if(panel)panel.hidden=true;
  legacy.querySelectorAll('[data-finance-help]').forEach(button=>button.setAttribute('aria-expanded','false'));
 }
 legacy.addEventListener('click',event=>{
  if(event.target.closest('[data-income-details]')){go('income');return;}
  const button=event.target.closest('[data-finance-help]'),close=event.target.closest('[data-finance-help-close]');
  if(close){const owner=legacy.querySelector('[data-finance-help][aria-expanded="true"]');closeFinanceHelp();owner?.focus();return;}
  if(!button)return;
  const wasOpen=button.getAttribute('aria-expanded')==='true';closeFinanceHelp();if(wasOpen)return;
  legacy.querySelectorAll('details.finance-info[open],details.tax-info[open]').forEach(node=>node.open=false);
  const panel=document.getElementById('financeHelp');
  panel.innerHTML=`<strong>${esc(button.getAttribute('aria-label').replace(/ erklären$/,''))}</strong><p>${esc(button.dataset.financeHelp)}</p><button type="button" data-finance-help-close>Hinweis schliessen</button>`;
  const anchor=button.closest('.finance-pots,.finance-capital-header,.finance-bound-secondary,.finance-method-help,.finance-compact-title,.finance-budget');
  anchor.after(panel);panel.hidden=false;button.setAttribute('aria-expanded','true');
 });
 legacy.addEventListener('keydown',event=>{const button=event.target.closest('[data-finance-help]');if(button&&['Enter',' '].includes(event.key)){event.preventDefault();button.click();return;}if(event.key==='Escape'){const owner=legacy.querySelector('[data-finance-help][aria-expanded="true"]');if(owner){closeFinanceHelp();owner.focus();event.preventDefault();}}});
 legacy.addEventListener('toggle',event=>{if(event.target.open&&event.target.matches('#tab-pots details')){closeFinanceHelp();legacy.querySelectorAll('#tab-pots details[open]').forEach(node=>{if(node!==event.target)node.open=false;});}},true);
