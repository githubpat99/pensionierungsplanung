...
assert.equal(M.quality(taxState),0,'a plan without a canton is no longer a complete calculation');
assert.ok(M.error('regular',{...taxState.details.income,canton:''},taxState,{requireCanton:true}),'new input requires the canton');
assert.equal(M.error('regular',{...taxState.details.income,canton:'AR'},taxState,{requireCanton:true}),null);
assert.equal(M.complete(taxState),false);
assert.equal(M.complete(taxState,{requireCanton:false}),true,'an older stand without a canton stays readable');
taxState=M.apply(taxState,'assumptions',{...M.defaults,targetAge:taxState.targetAge,reviewed:true});
assert.equal(M.quality(taxState),0,'missing canton prevents any confirmed data level');
